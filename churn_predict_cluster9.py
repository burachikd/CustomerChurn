...
K.set_epsilon(1e-10)
today = date.today()
models = os.path.join(os.getcwd(), 'Churn', 'models', 'Cluster9')
cluster = 9

print(models)

# if segmentation done in 1 july file named june2020
file = 'test_data_july2020.csv'
...
def scaler_load(test):
    scaler = load(open(os.path.join(models, '2020-05-23_segment_9.pkl'), 'rb'))
...
test_results_df2.to_excel(os.path.join(os.getcwd(), 'Churn', 'output', 'Cluster9', 'potential_churn_cluster9_' +
                                       churn_month_start + '.xlsx'))
churn_before.to_excel(os.path.join(os.getcwd(), 'Churn', 'output', 'Cluster9', 'churn_before_cluster9_' +
                                   churn_month_start + '.xlsx'))
