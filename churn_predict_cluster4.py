...
K.set_epsilon(1e-10)
today = date.today()
models = os.path.join(os.getcwd(), 'Churn', 'models', 'Cluster4')
cluster = 4
...
def scaler_load(test):
    scaler = load(open(os.path.join(models, '2020-05-12_segment_4.pkl'), 'rb'))
...
def create_model_and_load_weights(train_x):
    ...
    model.load_weights(os.path.join(models, '2020-05-12_segment_4_LSTM24.h5'.format(today, cluster)))
    return model
...
test_results_df2.to_excel(os.path.join(os.getcwd(), 'Churn', 'output', 'Cluster4', 'potential_churn_cluster4_' +
                                       churn_month_start + '.xlsx'))
churn_before.to_excel(os.path.join(os.getcwd(), 'Churn', 'output', 'Cluster4', 'churn_before_cluster4_' +
                                   churn_month_start + '.xlsx'))
